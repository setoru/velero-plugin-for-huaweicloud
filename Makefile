...
BIN ?= velero-plugin-for-huaweicloud

# This repo's root import path (under GOPATH).
PKG := github.com/vmware-tanzu/velero-plugin-for-huaweicloud

# Where to push the docker image.
REGISTRY ?= velero
HW_REGISTRY ?= hw.io/velero-hw

# Image name
IMAGE ?= $(REGISTRY)/$(BIN)
HW_IMAGE ?= $(HW_REGISTRY)/$(BIN)
...
ifeq ($(TAG_LATEST), true)
	...
	HW_IMAGE_TAGS ?= $(HW_IMAGE):$(VERSION) $(HW_IMAGE):latest
else
	IMAGE_TAGS ?= $(IMAGE):$(VERSION)
	HW_IMAGE_TAGS ?= $(HW_IMAGE):$(VERSION)
endif
